...
extends Component {
    renderCell(item, campo){
        ...
        if (campo.content) return <td key={campo.path} colSpan={campo.colSpan} >{campo.content(item)}</td>;
        //campo.content es una función, y se la llama con el argumento item
        return <td key={campo.path} colSpan={campo.colSpan}>{_.get(item , campo.path)} 
            </td>
    }
    render() { 
        ...
        return ( <tbody>
                     {listaElementos.map(item =><tr key = {item[campoClave] }> 
